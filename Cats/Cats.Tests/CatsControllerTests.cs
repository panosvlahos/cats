...
namespace Cats.Tests
{
    ...
    public class CatsControllerTests
    {
        ...
        [Fact]
        public async Task Fetch_ReturnsOkObjectResult_WithJobId()
        {
            // Arrange
            var expectedJobId = "test-job-id";
            _mediatorMock
                .Setup(m => m.Send(It.IsAny<FetchCatsCommand>(), It.IsAny<CancellationToken>()))
            ...
            // Use reflection to get the value of the anonymous type
            var jobIdProperty = okResult.Value?.GetType().GetProperty("jobId");
            var actualJobId = jobIdProperty?.GetValue(okResult.Value)?.ToString();
